...
```{r}
keywords30 <- head(sorted, n=30)

ggplot(keywords30,aes(fill=keywords30$Keyword)) +
  geom_bar(aes(x=keywords30$Keyword,y=keywords30$Freq),stat='identity',fill="pink")+
  xlab('Top 30 key words')+
  ylab('Frequency')+
  ggtitle('Frequency of top 30 key words')+
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) +
  scale_x_discrete(limits= keywords30$Keyword) + 
  guides(fill=FALSE)
...
sorted_s <- wordcount %>% dplyr::arrange(desc(Freq))
...
ggplot(keywords20,aes(fill=keywords30$Keyword)) +
  ...
  xlab('Top 20 key words')+
  ylab('Frequency')+
  ggtitle('Top 20 key words for Single non-parent')+
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) +
  scale_x_discrete(limits= keywords20$Keyword) + 
  guides(fill=FALSE)
```
  
## Top words for Married without children.
```{r}
wordcount <- hm_data.m$text %>% 
  str_trim() %>% 
  lapply(strsplit," ") %>% 
  unlist() %>% 
  table() %>% 
...
```{r}
demo.vec <- c("Single with children", "Married with children", "Single non-parent", "Married without children")
...
We use a heat map to visualized which topic is mentioned more frequently for each demographic group. From the heat map, "friend" is a topic that are mentioned the most for all four groups. Married parents seems to find more happiness in families than the other three groups.
